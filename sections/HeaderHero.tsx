  ...
  return (
    <header class="relative w-screen h-screen bg-primary overflow-x-hidden">
      <Image  
          ...
          src={backgroundImage}
      />
      <div class="relative top-0 left-10 lg:left-0  lg:px-[9%] pt-[7%] lg:pt-[1%] pb-[4%] w-full h-full">
        <div
            class="h-[12%] lg:h-1/2 border border-white absolute top-0 left-[70%] lg:left-[43%] z-20"
        />
        <div class="relative top-0 left-0 w-full h-[90%] lg:h-full z-10 flex items-start lg:items-center rounded-[34px] pl-6"
            style={{
        ...
        <div class="absolute bottom-[15%] z-20 max-w-[300px] lg:max-w-[500px] left-[10%] lg:left-[25%]">
            <p class="text-base lg:text-2xl leading-8 lg:leading-[34px] text-white pb-4 lg:pb-0">{description}</p>
            <h1 class="text-[38px] lg:text-[55px] leading-[32px] lg:leading-[83.66px] text-white font-black uppercase">{title}</h1>
        </div>
        <div class="overflow-x-auto bg-primary w-[90%] mx-auto rounded-[21px] relative z-20 lg:bottom-[8%] flex justify-between px-5">
            {
              items.map(item => (
